...
local function PPCheck( Context, Entity )
	...
	if not IsValid( Context.player ) then return false end
...
Component:AddVMFunction( "hasInput", "wl:s", "b", function( Context, Trace, WireLink, Index )
		return ( IsValid( WireLink ) and WireLink.Inputs and WireLink.Inputs[Index] ) and true or false 
	end )

Component:AddVMFunction( "hasOutput", "wl:s", "b", function( Context, Trace, WireLink, Index )
		return ( IsValid( WireLink ) and WireLink.Outputs and WireLink.Outputs[Index] ) and true or false 
	end )

Component:AddVMFunction( "isHiSpeed", "wl:", "b", function( Context, Trace, WireLink )
		return ( IsValid( WireLink ) and ( WireLink.WriteCell or WireLink.ReadCell ) ) and true or false 
	end )

Component:AddVMFunction( "inputType", "wl:s", "s", 
	function( Context, Trace, WireLink, Index )
		if IsValid( WireLink ) and WireLink.Inputs and WireLink.Inputs[Index] then 
...
Component:AddVMFunction( "outputType", "wl:s", "s", 
	function( Context, Trace, WireLink, Index )
		if IsValid( WireLink ) and WireLink.Outputs and WireLink.Outputs[Index] then 
...
local function WriteCell( Context, Trace, WireLink, Address, Value )
	if not IsValid( WireLink ) or not WireLink.WriteCell then return end
	return WireLink:WriteCell( Address, Value ) or false
end

local function ReadCell( Context, Trace, WireLink, Address )
	if not IsValid( WireLink ) or not WireLink.ReadCell then return 0 end
...
local function ReadArray( Context, Trace, WireLink, Start, End )
	...
	if not IsValid( WireLink ) or not WireLink.ReadCell then return Array end
...
local function WriteStringZero( Context, Trace, WireLink, Address, String )
	if not IsValid( WireLink ) or not WireLink.WriteCell then return 0 end
	
	if not WireLink:WriteCell( Address + #String, 0 ) then return 0 end
	
	for I = 1, #String do
		if not WireLink:WriteCell( Address + I - 1, String_Byte( String, I ) ) then
			return 0
		end
	end
...
local function ReadStringZero( Contex, Trace, WireLink, Address )
	if not IsValid( WireLink ) or not WireLink.ReadCell then return "" end
...
function EXPADV.linkWireIO( Context, Trace, Dst, DstId, Src, SrcId )
	if not IsValid( Dst ) or not WireLib.HasPorts( Dst ) or not Dst.Inputs then Context.Throw( Trace, "linkWireIO", tostring( Dst ) .." has no wire inputs" ) end
	if not IsValid( Src ) or not WireLib.HasPorts( Src ) or not Src.Outputs then Context.Throw( Trace, "linkWireIO", tostring( Src ) .." has no wire outputs" ) end
	if not Dst.Inputs[DstId] then Context.Throw( Trace, "linkWireIO", tostring( Dst ) .." has no `".. DstId .."` input" ) end
	if not Src.Outputs[SrcId] then Context.Throw( Trace, "linkWireIO", tostring( Src ) .." has no `".. SrcId .."` output" ) end
	if not PPCheck( Context, Dst ) or not PPCheck( Context, Src ) then return end
...
--NORMAL:
	Array:AddVMOperator( "get", "wa,n,n", "n",
		function( Context, Trace, Array, Index )
			if not isnumber( Array[Index] ) then return 0 end
			return Array[Index]
		end )
...
--VECTOR:
	Array:AddVMOperator( "get", "wa,n,v", "v",
		function( Context, Trace, Array, Index )
			...
			if not isvector( Value ) and not ( istable( Value ) and #Value == 3 ) then return Vector( 0,0,0 ) end
...
--ANGLE:
	Array:AddVMOperator( "get", "wa,n,a", "a",
		function( Context, Trace, Array, Index )
			local Value = Array[Index]
			if not isangle( Value ) and not ( istable( Value ) and #Value == 3 ) then return Angle( 0,0,0 ) end
...
--COLOR:
	Array:AddVMOperator( "get", "wa,n,c", "c",
		function( Context, Trace, Array, Index )
			local Value = Array[Index]
			if not IsColor( Value ) and not ( istable( Value ) and #Value == 4 ) then return Color( 0,0,0,255 ) end
...
--ENTITY:
	Array:AddVMOperator( "get", "wa,n,c", "c",
		function( Context, Trace, Array, Index )
			if not isentity( Array[Index] ) then return Entity( 0 ) end
			return Array[Index]
		end )
...
--STRING:
	Array:AddVMOperator( "get", "wa,n,s", "s",
		function( Context, Trace, Array, Index )
			if not isstring( Array[Index] ) then return "" end
			return Array[Index]
		end )
...
Component:AddVMFunction( "vectorArray", "wa", "ar",
	function( Context, Trace, WireArray )
		...
		for k, v in pairs( WireArray ) do
			if not ( isvector( v ) or ( istable( v ) and #v == 3 )) then continue end
...
Component:AddVMFunction( "vector2Array", "wa", "ar",
	function( Context, Trace, WireArray )
		...
		for k, v in pairs( WireArray ) do
			if not ( istable( v ) and #v == 2 ) then continue end
...
Component:AddVMFunction( "angleArray", "wa", "ar",
	function( Context, Trace, WireArray )
		...
		for k, v in pairs( WireArray ) do
			if not ( isangle( v ) or ( istable( v ) and #v == 3 )) then continue end
